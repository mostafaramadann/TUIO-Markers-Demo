...
using TUIO;
	...
	Dictionary<int, char> myDict = new Dictionary<int, char> { [0] = 'A', [1] = 'C', [4] = 'D', [5] = 'G', [9] = 'O' };
	Dictionary<int, char> myDict2 = new Dictionary<int, char> { [10] = 'R', [11] = 'T' };
	public static Bitmap screen = new Bitmap(640, 480);
	public TuioDemoObject (long s_id, int f_id, float xpos, float ypos, float angle) : base(s_id,f_id,xpos,ypos,angle) {
		...
		{
		Graphics g2 = Graphics.FromImage(TuioDemo.bmp);
		paint(g2);
		g.DrawImage(TuioDemo.bmp,0,0);
		}
	public void paint(Graphics g) {
		...
		/*if (words.Contains(word))
		{
				...
				Bitmap img = new Bitmap(T\word + "_"+TuioDemo.frameindex+".bmp");
				img.MakeTransparent();
				g.DrawImage(img, TuioDemo.lastx, TuioDemo.lasty);
		...
		else
		{
			...
			if (myDict.ContainsKey(symbol_id))
			{
				g.DrawString(myDict[symbol_id] + "", font, white, new PointF(Xpos - 10, Ypos - 10));
			}
			else if (myDict2.ContainsKey(symbol_id))
			{
				g.DrawString(myDict2[symbol_id] + "", font, white, new PointF(Xpos - 10, Ypos - 10));
			}
			else
